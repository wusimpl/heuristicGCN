...
class CompGCNConv(MessagePassing):
	def __init__(self, in_channels, out_channels, num_rels, act=lambda x:x, params=None):
		...
		self.w_loop		= get_param((in_channels, out_channels))  # Ws 实体更新时的关系权重矩阵（实为一线性层，下同）
		self.w_in		= get_param((in_channels, out_channels))  # Wi 实体更新的关系权重矩阵
		self.w_out		= get_param((in_channels, out_channels))  # Wo 实体更新的关系权重矩阵
		self.w_rel 		= get_param((in_channels, out_channels))  # Wrel 关系更新时的关系权重矩阵
	...
	def forward(self, x, edge_index, edge_type, rel_embed): 
		...
		self.in_index, self.in_type = edge_index[:, :num_edges], edge_type[:num_edges]

		self.out_index,self.out_type = edge_index[:, num_edges:], edge_type[num_edges:]

		self.loop_index  = torch.stack([torch.arange(num_ent), torch.arange(num_ent)]).to(self.device) # (2,14541)
		self.loop_type   = torch.full((num_ent,), rel_embed.size(0)-1, dtype=torch.long).to(self.device) # (14541,1)
	...
	def message(self, x_j, edge_type, rel_embed, edge_norm, mode):
		weight 	= getattr(self, 'w_{}'.format(mode)) # 对于每一种关系类型，都有一个关系权重矩阵
		rel_emb = torch.index_select(rel_embed, 0, edge_type) # 获取关系的表示
		xj_rel  = self.rel_transform(x_j, rel_emb)  # phi(x_u,z_r)
		out	= torch.mm(xj_rel, weight)  # w_r*phi(~)
